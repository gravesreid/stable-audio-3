"""Tests for the focus playlist prompt bank (focus_prompts.py).

The bank's whole promise is "240 distinct prompts, none of which sing", and both
halves of that are easy to break by hand-editing a pool. These tests are pure
data checks -- no model, no torch, no weights.
"""
...
from focus_prompts import (  # noqa: E402
    FAMILIES,
    NEGATIVE_PROMPT,
    build_prompts,
    capacity,
...
# 8 hours at the 120s per-track ceiling of the small-music model.
EIGHT_HOURS = 240
...
def test_capacity_covers_a_full_working_day():
    assert capacity() >= EIGHT_HOURS
...
def test_eight_hours_of_prompts_are_all_distinct():
    prompts, _ = build_prompts(EIGHT_HOURS, random.Random(0))
    assert len(prompts) == EIGHT_HOURS
    assert len(set(prompts)) == EIGHT_HOURS


def test_prompts_are_distinct_at_full_capacity():
    """The uniqueness claim should hold at the advertised ceiling, not just at 240."""
    prompts, _ = build_prompts(capacity(), random.Random(0))
    assert len(set(prompts)) == capacity()


@pytest.mark.parametrize("n", [1, 7, 30, 100, EIGHT_HOURS, 1000])
...
def test_seed_is_reproducible():
    a, _ = build_prompts(EIGHT_HOURS, random.Random(42))
    b, _ = build_prompts(EIGHT_HOURS, random.Random(42))
    assert a == b


def test_different_seeds_give_different_playlists():
    a, _ = build_prompts(EIGHT_HOURS, random.Random(1))
    b, _ = build_prompts(EIGHT_HOURS, random.Random(2))
    assert a != b
...
def test_families_are_interleaved_not_clumped(seed):
    ...
    prompts, _ = build_prompts(EIGHT_HOURS, random.Random(seed))
